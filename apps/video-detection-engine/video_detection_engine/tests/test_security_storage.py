...
def test_tenant_isolation(clean_storage):
    ...
    # Ensure they start with root
    assert t1_path.startswith(os.path.realpath(clean_storage))
...
def test_save_file_traversal(clean_storage):
    ...
    with pytest.raises(SecurityError):
         # We assume 'dummy' content provider
         class Content:
             def read(self): return b""
         
         store.save_file("t1", "v1", "../../evil.exe", Content())
