...
export interface CustomerServiceToneResult {
    customer_service_tone_count: number;
    customer_service_tone_hits: string[];
}
...
// ---------------------------------------------------------------------------
// XHS customer-service tone detector
// ---------------------------------------------------------------------------

// Customer-service closing phrases that break the XHS human-comment voice.
const XHS_CS_PHRASES = [
    '随时问我',
    '随时告诉我',
    '欢迎了解',
    '欢迎咨询',
...
// Trailing question: reply ends with a Chinese or ASCII question mark (ignoring trailing spaces/emoji).
const TRAILING_QUESTION_RE = /[？?]\s*[\u{1F300}-\u{1FFFF}☀-➿]*\s*$/u;

// Excessive emoji: 3 or more emoji characters in the reply.
const EMOJI_RE = /[\u{1F300}-\u{1FFFF}☀-➿]/gu;

/**
 * Detects customer-service tone patterns in a reply for Xiaohongshu.
 * Only applies XHS-specific checks when platform is 'xiaohongshu' or 'xhs'.
 * Returns empty result for all other platforms.
 */
export function detectCustomerServiceTone(
    replyText: string,
    platform: string,
): CustomerServiceToneResult {
    const normalizedPlatform = platform.toLowerCase().trim();
    if (normalizedPlatform !== 'xiaohongshu' && normalizedPlatform !== 'xhs') {
        return { customer_service_tone_count: 0, customer_service_tone_hits: [] };
    }

    const hits: string[] = [];

    for (const phrase of XHS_CS_PHRASES) {
        if (replyText.includes(phrase)) {
            hits.push(`CS phrase: "${phrase}"`);
        }
    }

    if (TRAILING_QUESTION_RE.test(replyText.trim())) {
        hits.push('trailing question ending');
    }

    const emojiMatches = [...replyText.matchAll(EMOJI_RE)];
    if (emojiMatches.length >= 3) {
        hits.push(`excessive emoji: ${emojiMatches.length} found`);
    }

    return {
        customer_service_tone_count: hits.length,
        customer_service_tone_hits: hits,
    };
}
