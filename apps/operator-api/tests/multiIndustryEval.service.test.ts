...
// ---------------------------------------------------------------------------
// P10.4 — detectCustomerServiceTone tests
// ---------------------------------------------------------------------------

test('detectCustomerServiceTone flags "如果还有其他问题…随时问我" composite phrase', () => {
    const result = detectCustomerServiceTone(
        '有 UV400，日常防晒够用。如果还有其他问题或者想了解更多，可以随时问我！',
        'xiaohongshu',
    );
    assert.ok(result.customer_service_tone_count > 0);
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('如果还有其他问题')));
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('随时问我')));
});

test('detectCustomerServiceTone flags trailing question ending "你更看重哪方面呢？"', () => {
    const result = detectCustomerServiceTone(
        '和小米相比我们的 UV400 防护更完善。你更看重哪方面呢？',
        'xiaohongshu',
    );
    assert.ok(result.customer_service_tone_count > 0);
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('你更看重哪方面')));
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('trailing question')));
});

test('detectCustomerServiceTone flags "有兴趣的话" phrase', () => {
    const result = detectCustomerServiceTone(
        '这款精华不含酒精。有兴趣的话可以看看其他成分哦。',
        'xiaohongshu',
    );
    assert.ok(result.customer_service_tone_count > 0);
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('有兴趣的话')));
});

test('detectCustomerServiceTone flags "欢迎咨询" phrase', () => {
    const result = detectCustomerServiceTone(
        '敏感肌可以先局部试。欢迎咨询我们获取更多建议。',
        'xiaohongshu',
    );
    assert.ok(result.customer_service_tone_count > 0);
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('欢迎咨询')));
});

test('detectCustomerServiceTone flags excessive emoji (3+)', () => {
    const result = detectCustomerServiceTone(
        '有 UV400 防护 ✨ 日常够用啦 🌞 戴上很好看 😊 随时来问我',
        'xiaohongshu',
    );
    assert.ok(result.customer_service_tone_count > 0);
    assert.ok(result.customer_service_tone_hits.some((h) => h.includes('excessive emoji')));
});

test('detectCustomerServiceTone does not flag a short natural factual reply', () => {
    const result = detectCustomerServiceTone(
        '有 UV400，日常防晒够用。不是偏光，开车反光场景会没那么舒服。',
        'xiaohongshu',
    );
    assert.strictEqual(result.customer_service_tone_count, 0);
    assert.deepStrictEqual(result.customer_service_tone_hits, []);
});

test('detectCustomerServiceTone does not flag a natural purchase reply', () => {
    const result = detectCustomerServiceTone(
        '主页有链接，点进去就能看价格。',
        'xiaohongshu',
    );
...
test('detectCustomerServiceTone returns empty result for non-XHS platform', () => {
    const result = detectCustomerServiceTone(
        'Great question! Feel free to reach out if you have any other questions.',
        'website_chat',
    );
    assert.strictEqual(result.customer_service_tone_count, 0);
    assert.deepStrictEqual(result.customer_service_tone_hits, []);
