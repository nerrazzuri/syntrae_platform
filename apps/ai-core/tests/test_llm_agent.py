...
class TestLLMAgent(unittest.TestCase):
    ...
    def test_model_uses_env_config(self):
        with patch.dict(
            os.environ,
            {"LLM_MODEL": "", "RAG_CHAT_MODEL": "gpt-test-model"},
            clear=False,
        ):
            agent = LLMAgent()
    ...
    def test_plan_redacts_pii_before_model_call(self, MockOpenAI):
        mock_client = MagicMock()
        MockOpenAI.return_value = mock_client

        resp1 = MagicMock()
        resp1.choices[0].message.content = "Thought: I have the answer.\nFinal Answer: ok"

        resp2 = MagicMock()
        resp2.choices[0].message.content = "ok"

        mock_client.chat.completions.create.side_effect = [resp1, resp2]

        self.agent.plan("Contact alice@example.com", {})

        first_call = mock_client.chat.completions.create.call_args_list[0]
        user_message = first_call.kwargs["messages"][1]["content"]
        self.assertEqual(user_message, "Contact [REDACTED_EMAIL]")
