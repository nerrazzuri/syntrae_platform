...
router.post('/register', async (req: Request, res: Response) => {
    try {
        ...
        // Optional: IP Rate Limit for Registration (same bucket as login or stricter?)
        // For now, skip or use login bucket to prevent spam?
        // RateLimitService.check(req.ip || '127.0.0.1');

        const result = await AuthService.register(email, password, workspaceName);
    ...
    try {
        ...
        res.json({
            ...
            user: {
                id: result.user.id,
                email: result.user.email,
                name: "User" // Placeholder name field if not in DB yet
            },
