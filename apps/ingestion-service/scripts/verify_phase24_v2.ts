...
async function main() {
    ...
    const workspaceId = wsData.account.id; // Corrected extraction
    ...
    const eventPayload = {
        ...
        platform: 'youtube', // Allowed enum
        video: {
            video_id: 'vid_24',
            title: 'Phase 24 Test',
            author_name: 'Creator',
            video_url: 'http://yt.com/watch?v=vid_24',
            author_id: 'creator_id_1'
        },
        comment: {
            comment_id: 'c_24_1',
            text: 'Limits Test?',
        ...
        page: {
            url: 'http://yt.com/watch?v=vid_24',
            page_type: 'VIDEO',
            timestamp: new Date().toISOString()
        },
        session: {
            session_id: uuid.v4(),
            install_id: installId // Required in session too
        },
        context: {
            visible: true,
            position: 'viewport',
